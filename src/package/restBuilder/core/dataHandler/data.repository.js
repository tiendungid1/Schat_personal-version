...
import { ArrayUtils } from 'package/utils';
import { BUILDER_TYPE } from '../../enum/build-type.enum';
...
export class DataRepository {
    ...
    constructor(model) {
        ...
        LoggerFactory.globalLogger.info(
            `[${upperFirst(
                camelCase(model.collection.collectionName),
            )}Repository] is bundling`,
        );
    }
     ...
     * @param {import('../../enum/build-type.enum').BUILDER_TYPE} querySelector
     */
    getTemplateQuery(
        pagination,
        filter,
        ...
        main,
        associates,
        querySelector = BUILDER_TYPE.FIND,
    ) {
        return QueryBuilder.builder()
            [querySelector](this.model)
            .addFilter(filter)
    ...
    getAndCount(pagination, filter, sort, search, main, associates) {
        const baseQuery = this.getTemplateQuery(
            pagination,
            filter,
            sort,
            search,
    ...
    findByIds(ids, fields = '') {
        return this.model.find({ _id: { $in: ids } }, fields).lean();
    }
    ...
    updateByIds(ids, fields) {
        return this.model.updateMany({ _id: { $in: ids } }, { ...fields });
    }
