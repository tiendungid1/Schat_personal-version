...
import { parallel, serial } from 'package/taskExecution';
...
export class BaseProcessor {
    ...
    preProcess(tasks) {
        throw new Error(
            'Class extends BaseProcessor is not implemented method preProcess',
        );
    }
    ...
    afterProcess() {
        throw new Error(
            'Class extends BaseProcessor is not implemented method afterProcess',
        );
    }
    ...
    afterRun(task) {
        throw new Error(
            'Class extends BaseProcessor is not implemented method afterRun',
        );
    }
        ...
        await serial(this.tasks, async task => {
            try {
                if (await this.preRun(task)) {
                    console.log(
                        `\x1B[92m🐧 ========== Running task: ${task.name} ===========🐧`,
                    );
                    await task.run();
                    console.log(
